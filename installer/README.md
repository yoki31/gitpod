...
```shell
./installer init > /path/to/gitpod.config.yaml
```
...
./installer validate config --config /path/to/gitpod.config.yaml
```
...
./installer validate cluster --kubeconfig /path/to/kubeconfig --config /path/to/gitpod.config.yaml
```
...
```shell
./installer render --config /path/to/gitpod.config.yaml > /path/to/gitpod.yaml
```

## Deploy

```shell
kubectl apply -f /path/to/gitpod.yaml
```
