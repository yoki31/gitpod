...
export class GitpodServerImpl<Client extends GitpodClient, Server extends GitpodServer> implements GitpodServer, Disposable {
    ...
    public async createRepositoryFromTemplate(params: CreateRepositoryFromTemplateParams): Promise<Repository> {
        ...
        const hostContext = this.hostContextProvider.get(params.provider);
        if (!hostContext || !hostContext.services) {
            throw new Error('Could not create project from template: Unsupported repository provider');
        }
        const repoProvider = hostContext.services.repositoryProvider;
        return await repoProvider.createRepoFromTemplate(user, params.owner, params.repo, params.templateUrl);
    }
