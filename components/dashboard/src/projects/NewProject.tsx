...
export default function NewProject() {
    ...
    useEffect(() => {
        if (reposInAccounts.length === 0) {
            ...
            const mostRecent = reposInAccounts.reduce((prev, current) => prev.installationUpdatedAt > current.installationUpdatedAt ? prev : current);
    ...
    const getDropDownEntries = (accounts: string[]) => {
        const result: ContextMenuEntry[] = [];
        for (const account of accounts) {
            result.push({
                title: account,
                customContent: <span className="text-gray-800 dark:text-gray-100 text-base font-semibold">{account}</span>,
                separator: true,
        ...
        if (isGitHub()) {
            result.push({
                ...
                customContent: <span className="text-gray-800 dark:text-gray-100 text-base">Add GitHub Orgs or Account</span>,
                separator: true,
                onClick: () => reconfigure(),
            })
        }
        result.push({
            ...
            customContent: <span className="text-gray-800 dark:text-gray-100 text-base">Select Git Provider</span>,
            ...
            <div className="px-8 pt-8 flex flex-col space-y-2">
                <ContextMenu classes="w-full left-0 cursor-pointer" menuEntries={getDropDownEntries(accounts)}>
                    <div className="w-full relative ">
                        <input className="w-96 pr-8 cursor-pointer" readOnly type="text" value={selectedAccount || ""}></input>
            ...
            <div className="p-6 flex-col">
                {reposToRender.length > 0 && (
                    <div className="overscroll-contain max-h-80 overflow-y-auto pr-2">
                        {reposToRender.map(r => (
                            <div key={`repo-${r.name}`} className="cursor-pointer rounded-xl whitespace-nowrap flex space-x-2 py-2 px-3 w-full justify-between hover:bg-gray-100 dark:hover:bg-gray-800 focus:bg-gitpod-kumquat-light transition ease-in-out group" onClick={() => setSelectedRepo(r.name)}>
